...
standalone ``colosseum run`` (``debug.log``, ``execution.sqlite``, ``summary.txt``, and
a ``-pass`` / ``-fail`` directory rename). Supporting slots keep a timestamped folder
without a result suffix.
