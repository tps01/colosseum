...
Colosseum test cases are ordinary Python scripts. Installed plugins expose bench APIs
as ``col.<namespace>.*``. This chapter walks through a complete script with matching
bench TOML and metadata YAML. Install ``colosseum-core`` and ``colosseum-template`` first
...
``../scripts/my_test.py``). Bench config and metadata paths are passed on the CLI or
...
2. Load bench config (and optional metadata) before calling plugin APIs when running
...
Direct-Python skeleton (script loads config and calls ``endex()``)::
   ...
   def main() -> None:
       col.config.load_config("config/bench.toml")
...
Bench TOML describes devices, instruments, and other rows owned by installed plugins.
Load it before any ``col.*`` API call::

   col.config.load_config("bench.toml")

or pass ``--config bench.toml`` to ``colosseum run``.

The example below uses the ``colosseum_template`` plugin (namespace ``template``).
Each ``[[template.device]]`` row matches that plugin's ``ConfigSectionSpec``:
...
WATS metadata (optional)
------------------------

WATS reports identify the unit under test and the test run. Metadata is optional:
Colosseum writes a best-effort WATS JSON file even when metadata is not configured.

Provide metadata in one of two ways:

* A ``[colosseum.metadata]`` table in bench TOML (see :doc:`configuration`).
...
The files below describe one bench run.
...
CLI (recommended; loads config and metadata from flags)::
   ...
   colosseum run scripts/my_test.py --config config/bench.toml --metadata config/metadata.yaml
...
With metadata omitted, WATS still exports using runtime defaults (hostname, OS user,
empty identity fields). See :doc:`output_artifacts`.
...
``no_artifacts=True`` or pass ``--no-artifacts`` on the CLI::
   ...
   col.config.load_config("bench.toml", no_artifacts=True)

   # or
   colosseum run my_test.py --config bench.toml --no-artifacts
